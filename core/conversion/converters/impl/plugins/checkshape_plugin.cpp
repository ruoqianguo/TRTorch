...
inline bool CheckShapePlugin::supportsFormatCombination(int32_t pos, const PluginTensorDesc *inOut, 
    ...
    case 0: // input0
#ifndef NDEBUG
        printf("    id=0 inOut.type: %d, format: %d\n", inOut[0].type, inOut[0].format);
#endif
        return inOut[0].type==DataType::kINT32 && inOut[0].format==PluginFormat::kLINEAR;
    case 1: // input1
#ifndef NDEBUG
        printf("    id=1 inOut.type: %d, format: %d\n", inOut[1].type, inOut[1].format);
#endif
        return inOut[1].type==DataType::kINT32 && inOut[1].format==PluginFormat::kLINEAR;
    case 2: // output0
#ifndef NDEBUG
        printf("    id=2 inOut.type: %d, format: %d\n", inOut[2].type, inOut[2].format);
#endif
...
#endif
#ifndef NDEBUG
    printf("    inputDesc[0]: Dims(%d:%d), type:%d, float:%f.\n",
        inputDesc[0].dims.nbDims,inputDesc[0].dims.d[0],inputDesc[0].type, inputDesc[0].scale);
    printf("    inputDesc[1]: Dims(%d:%d), type:%d, float:%f.\n",
        inputDesc[1].dims.nbDims,inputDesc[1].dims.d[0],inputDesc[1].type, inputDesc[1].scale);
    printf("    outputDesc[0]: Dims(%d:%d,%d,%d,%d), type:%d, float:%f.\n",
        outputDesc[0].dims.nbDims,outputDesc[0].dims.d[0],outputDesc[0].type, outputDesc[0].scale);
#endif
    int status = 0;

  ...
  switch (pos) {
    case 0: // input0
      ...
      return (inOut[0].type == DataType::kFLOAT || inOut[0].type == DataType::kHALF) &&
          inOut[0].format == PluginFormat::kLINEAR;
...
int LayerNormPlugin::enqueue(
  ...
  at::Tensor weight = at::from_blob(
      (void*)inputs[1], util::toVec(inputDesc[1].dims), [](void*) {}, at::device(at::kCUDA).dtype(tensor_type));
  at::Tensor bias = at::from_blob(
      (void*)inputs[2], util::toVec(inputDesc[2].dims), [](void*) {}, at::device(at::kCUDA).dtype(tensor_type));
  at::Tensor output = at::from_blob(
      outputs[0], util::toVec(outputDesc[0].dims), [](void*) {}, at::device(at::kCUDA).dtype(tensor_type));

    ...
    RegisterNodeConversionPatterns()
        .pattern({"aten::constant_pad_nd(Tensor self, int[] pad, Scalar value=0) -> (Tensor)",
                  [](ConversionCtx* ctx, const torch::jit::Node* n, args& args) -> bool {
                    ...
                    TRTORCH_CHECK(inRank >= (int64_t)l_pad, "Length of pad should be no more than twice the number of "
              "dimensions of the input. Pad length is " << padSize << "while the input has " << inRank << "dimensions.");
                    ...
                    for (int64_t i = 0; i < l_pad; i++) {
                      ...
                      if (padding[padding_index + 1] > 0) { // right/bottom/back padding value
                        ...
                        if (inDims.d[axis] == -1) {
                          ...
                          indicesTensor = ctx->net->addElementWise(*indicesTensor, *oneTensor, nvinfer1::ElementWiseOperation::kSUB)->getOutput(0);
                        } else {
